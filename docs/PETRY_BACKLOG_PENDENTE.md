...
- [ ] **`components/site/BannerRotator.tsx` órfão** — desde o redesign de `LinhasSection` (`docs/PETRY_LINHAS_INDEX_REDESIGN.md`), nenhuma seção da home usa mais este componente (o Hero já não usava desde a Fase 1 dele). Candidato a remoção junto com as imagens que só ele consumia: `public/banners/linhas/*` (`c5.png`, `c5 (2).png`, `c7.png`, `c8.png`, `c10.png`, `c14.png`, `c16.png`, `c17.png`) e `public/banners/desktop/*` (`Banner12.jpg`, `Banner13.jpg`, `Banner15.jpg`, usados no `DEFAULT_SLIDES` do componente). Não apagado ainda — só sinalizado, pra virar decisão separada.
- [ ] **Mockups órfãos de `public/banners/catalogos/`** — desde o redesign de `CatalogosCinematic` (`docs/PETRY_CATALOGOS_CAROUSEL_REDESIGN.md`), que passou a usar as capas reais (`capa1.jpg`/`capa2.jpg`/`capa3.jpg`/`capa4.jpg`), os arquivos `ctg-perfil.jpg`, `ctg-perfetta.jpg`, `ctg-ace1.jpg` e `ctg-tecnico.jpg` (render "flat lay" genérico) não são mais referenciados em nenhum lugar do site. Candidatos a remoção.
- [ ] **Links do Google Drive duplicados em 2 lugares** — `components/site/produtos/ProdutosHero.tsx` (4 botões) e `components/site/catalogos/CatalogosGrid.tsx` mantêm a mesma lista de links hardcoded de forma independente (antes eram 3 lugares; `CatalogosCinematic` não guarda mais link direto desde o redesign do carrossel). Vale centralizar em `lib/catalogos.ts` (mesmo padrão do `lib/linhas.ts`), pra mudar uma URL do Drive em um lugar só.
- [ ] **`components/site/LinhasSection.tsx` e `lib/linhas.ts` órfãos** — desde `docs/PETRY_TUBOS_SHOWCASE_REDESIGN.md`, a home usa `TubosShowcase` (vitrine de tubos) no lugar do índice de 46 linhas. O import de `LinhasSection` está comentado em `app/(site)/page.tsx` (mesmo padrão do `AcabamentosSection`), não apagado — as outras ~40 linhas continuam acessíveis via `/produtos`. Candidato a remoção definitiva se o índice de linhas não voltar a ser usado em nenhum lugar.
